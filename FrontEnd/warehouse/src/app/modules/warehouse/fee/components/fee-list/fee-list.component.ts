...
export class FeeListComponent implements OnInit {
  ...
  reloadDataListener() {
    this.feeService.reloadGrid.subscribe(response => {
      if (response === true) {
        this.getlist();
      }
    });
  ...
  onAddNewClick() {
    const modalRef = this.modal.open(FeeFormComponent, AppLicationSetting.modalOptions.modalSmallOptions);
    modalRef.componentInstance.isEdit = false;
  }

  onUpdateClick(item: FeeModel) {
    const modalRef = this.modal.open(FeeFormComponent, AppLicationSetting.modalOptions.modalSmallOptions);
    modalRef.componentInstance.isEdit = true;
    modalRef.componentInstance.fee = item;
  }

  onDeleteClick(item: FeeModel) {
    this.confirmMessage.open().then(response => {
      if (response) {
        this.feeService.delete(item).subscribe((response: ResponseModel) => {
  ...
  private getlist() {
    ...
    this.feeService.list(this.filter).subscribe((response: ResponseModel) => {
      if (response.responseStatus === ResponseStatus.warning) {
        this.messageService.showWarning(response.errors.join(','));
