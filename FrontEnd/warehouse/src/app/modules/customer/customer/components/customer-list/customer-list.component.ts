...
export class CustomerListComponent implements OnInit {
  ...
  formMessage = CustomerResource.list;
  filter: FilterModel;
  list: CustomerModel[] = [];
  totalRow = 0;
  ...
  reloadDataListener() {
    this.customerService.reloadGrid.subscribe(response => {
      if (response === true) {
        this.getlist();
      }
    });
  ...
  onAddNewClick() {
    const modalRef = this.modal.open(CustomerFormComponent, AppLicationSetting.modalOptions.modalExtraOptions);
    modalRef.componentInstance.isEdit = false;
  }

  onUpdateClick(item: CustomerModel) {
    const modalRef = this.modal.open(CustomerFormComponent, AppLicationSetting.modalOptions.modalExtraOptions);
    modalRef.componentInstance.isEdit = true;
    modalRef.componentInstance.customer = item;
  }

  onDeleteClick(item: CustomerModel) {
    this.confirmMessage.open().then(response => {
      if (response) {
        this.customerService.delete(item).subscribe((response: ResponseModel) => {
  ...
  private getlist() {
    ...
    this.customerService.list(this.filter).subscribe((response: ResponseModel) => {
      if (response.responseStatus === ResponseStatus.warning) {
        this.messageService.showWarning(response.errors.join(','));
