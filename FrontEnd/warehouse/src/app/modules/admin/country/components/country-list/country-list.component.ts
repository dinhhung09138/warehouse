...
export class CountryListComponent implements OnInit {
  ...
  formMessage = CountryResource.list;
  filter: FilterModel;
  list: CountryModel[] = [];
  totalRow = 0;
  ...
  reloadDataListener() {
    this.countryService.reloadGrid.subscribe(response => {
      if (response === true) {
        this.getlist();
      }
    });
  ...
  onAddNewClick() {
    const modalRef = this.modal.open(CountryFormComponent, AppLicationSetting.modalOptions.modalSmallOptions);
    modalRef.componentInstance.isEdit = false;
  }

  onUpdateClick(item: CountryModel) {
    const modalRef = this.modal.open(CountryFormComponent, AppLicationSetting.modalOptions.modalSmallOptions);
    modalRef.componentInstance.isEdit = true;
    modalRef.componentInstance.country = item;
  }

  onDeleteClick(item: CountryModel) {
    this.confirmMessage.open().then(response => {
      if (response) {
        this.countryService.delete(item).subscribe((response: ResponseModel) => {
  ...
  private getlist() {
    ...
    this.countryService.list(this.filter).subscribe((response: ResponseModel) => {
      if (response.responseStatus === ResponseStatus.warning) {
        this.messageService.showWarning(response.errors.join(','));
