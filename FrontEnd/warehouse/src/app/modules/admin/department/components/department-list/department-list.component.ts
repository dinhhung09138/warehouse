...
export class DepartmentListComponent implements OnInit {
  ...
  formMessage = DepartmentResource.list;
  filter: FilterModel;
  ...
  reloadDataListener() {
    this.departmentService.reloadGrid.subscribe(response => {
      if (response === true) {
        this.getlist();
      }
    });
  ...
  onAddNewClick() {
    const modalRef = this.modal.open(DepartmentFormComponent, AppLicationSetting.modalOptions.modalSmallOptions);
    modalRef.componentInstance.isEdit = false;
  }

  onUpdateClick(item: DepartmentModel) {
    const modalRef = this.modal.open(DepartmentFormComponent, AppLicationSetting.modalOptions.modalSmallOptions);
    modalRef.componentInstance.isEdit = true;
    modalRef.componentInstance.department = item;
  }

  onDeleteClick(item: DepartmentModel) {
    this.confirmMessage.open().then(response => {
      if (response) {
        this.departmentService.delete(item).subscribe((response: ResponseModel) => {
  ...
  private getlist() {
    ...
    this.departmentService.list(this.filter).subscribe((response: ResponseModel) => {
      if (response.responseStatus === ResponseStatus.warning) {
        this.messageService.showWarning(response.errors.join(','));
