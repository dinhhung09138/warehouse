...
export class CityListComponent implements OnInit {
  ...
  reloadDataListener() {
    this.cityService.reloadGrid.subscribe(response => {
      if (response === true) {
        this.getlist();
      }
    });
  ...
  onAddNewClick() {
    const modalRef = this.modal.open(CityFormComponent, AppLicationSetting.modalOptions.modalSmallOptions);
    modalRef.componentInstance.isEdit = false;
  }

  onUpdateClick(item: CityModel) {
    const modalRef = this.modal.open(CityFormComponent, AppLicationSetting.modalOptions.modalSmallOptions);
    modalRef.componentInstance.isEdit = true;
    modalRef.componentInstance.city = item;
  }

  onDeleteClick(item: CityModel) {
    this.confirmMessage.open().then(response => {
      if (response) {
        this.cityService.delete(item).subscribe((response: ResponseModel) => {
  ...
  private getlist() {
    ...
    this.cityService.list(this.filter).subscribe((response: ResponseModel) => {
      if (response.responseStatus === ResponseStatus.warning) {
        this.messageService.showWarning(response.errors.join(','));
